...
function onDateRangeChange(args) {
    ...
    var st = args.startDate;
    var ed = args.endDate;
  //  var ajax = new ej.base.Ajax("/Home/updateRange?frmDate=" + st.toISOString().split('T')[0] + "&toDate=" + ed.toISOString().split('T')[0], 'GET');
  //var ajax = new ej.base.Ajax("/Home/updateRange?frmDate=" + st.toISOString().split('T')[0] + "&toDate=" + ed.toISOString().split('T')[0], 'GET', true);

    //ajax.send();
    //ajax.onSuccess = function() {
    //    alert("Success!");
        //obj = document.getElementById("tolexpense").ej2_instances[0];
        //document.getElementById("tolexpense").textContent = data;
        //obj.redraw();

    //};
    var frm = st.toISOString().split('T')[0];
    var to = ed.toISOString().split('T')[0];

    var Ajax = new ej.base.Ajax({
        type: "POST",
        //type: "GET",
        //url: "/Home/updateRange",
        url: "/Home/updateRange?frmDate=" + st.toISOString().split('T')[0] + "&toDate=" + ed.toISOString().split('T')[0],
    ...
    Ajax.onSuccess = function (result) {
        ...
        document.getElementById("tolexpense").textContent = data.rev;
        document.getElementById("tolincome").textContent = data.avg;
        document.getElementById("tolbalance").textContent = data.ship;
        document.getElementById("toltransaction").textContent = data.net;
        pie = document.getElementById("total-expense").ej2_instances[0];
        pie.series[0].dataSource = data.pieDataSource;
        pie.refresh();
        grid = document.getElementById("legend-grid").ej2_instances[0];
        grid.dataSource = data.pieDataSource;
        grid.refresh();
        linechartObj = document.getElementById("balance").ej2_instances[0];
        linechartObj.series[0].dataSource = data.lineDS;
        linechartObj.refresh();
        
        linechartObj = document.getElementById("account-balance").ej2_instances[0];
        linechartObj.series[0].dataSource = data.barDS;
        linechartObj.series[1].dataSource = data.barDS;
        linechartObj.refresh();
        
        //grid = document.getElementById("recentexpense-grid").ej2_instances[0];
        //grid = document.getElementById("recentexpense-grid").ejGrid("instance");
        //grid.series[0].dataSource = data.dsTable;
        //grid.ejGrid("dataSource") = data.dsTable;
         gridObj = document.getElementById("recentexpense-grid").ej2_instances[0];
        gridObj.dataSource = data.dsTable;
        //gridObj.query = new ej.data.Query().where(predicate).sortByDesc('DateTime').take(5);
        gridObj.query = new ej.data.Query().take(5);
        gridObj.refresh();
        //grid.dataSource(data.dsTable);
        //grid.refresh();
    };
    ...
    //alert(" start date is : " + st.toISOString().split('T')[0] + " \n end date is : " + ed.toISOString().split('T')[0]);
    /*
    lineDS = [];
    lineD = [];
    columnIncomeDS = [];
...
function InitializeComponet() {
    ...
    new ej.data.DataManager(window.expenseDS)
        ...
        .then(function (e) {
            ...
            //document.getElementById("testel").innerHTML = JSON.stringify(lineDS);
